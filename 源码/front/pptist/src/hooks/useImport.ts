...
export default () => {
  ...
  // 导入PPTX文件
  const importPPTXFile = async(files: FileList) => {
    ...
    try{
      const response = await axios.post(
        'http://111.229.20.120:10010/video/uploadPPT',
        {
          file:file
        },
        {
      ...
      slidesStore.setSlidesId(response.data.data.fileId)
    }catch(error){
      console.error('error happen in upload ppt file',error)
    }
  }
